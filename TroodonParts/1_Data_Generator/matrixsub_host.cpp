...
int main(int argc, char** argv)
{
   ...
   int _Device = atoi(argv[2]); 

   int _Device_Platform = atoi(argv[3]);

   string _Features = argv[4]; 
   ...
   // Connect to a compute device
    /****************************************** 2. Device & Platform Setting *********************************************/

   err = clGetDeviceIDs(platform_ids[_Device_Platform], _Device ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU, 1, &device_id, NULL);
   ...
   lFileSize = LoadOpenCLKernel("matrixsub_kernel.cl", &KernelSource);
   if( lFileSize < 0L ) {
   ...
   printf("Running matrix subtraction for matrices A (%dx%d) and B (%dx%d) ...\n", WA,HA,WB,HB); 
   ...
   diff_t = (time_end - time_start);
   diff_t = diff_t/CLOCKS_PER_SEC;
   

   printf("Execution time = %f\n", diff_t);//Shutdown and cleanup
   ...
   fout.open("../Data/1/Execution_Data.csv", ios::out | ios::app);
   fout << _Features << ",";
   fout << _Device << ",";
   fout << diff_t << "\n";
   fout.close();
