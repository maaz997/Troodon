...
int main(int argc, char** argv)
{
   ...
   fstream fin;
   fin.open("../Matrix_Sizes.txt", ios::in | ios::app);
   while (!fin.eof( )) {
        int value; 
        fin>> value;
   ...
   int gpu = 0;
   err = clGetDeviceIDs(platform_ids[1], gpu ? CL_DEVICE_TYPE_GPU : CL_DEVICE_TYPE_CPU, 1, &device_id, NULL);
   if (err != CL_SUCCESS)
   {
  ...
  clock_t time_start[3],time_end[3];
   float diff_t[3];
 for (int i=0;i<3;i++){
       time_start[i]=clock();
        ...
        */
        ...
        diff_t[i] = (time_end[i] - time_start[i]);
        diff_t[i] = diff_t[i]/CLOCKS_PER_SEC;
        printf("Execution time = %f\n", diff_t[i]);//Shutdown and cleanup
    ...
    fout << "Matrix subtraction , CPU " << ", " << value << ", " << diff_t[0] << ", " << diff_t[1] << ", " << diff_t[2] << "\n";
