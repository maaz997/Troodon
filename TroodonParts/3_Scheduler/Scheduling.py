...
'''
with open(base+"/Data/3/"+str(id)+".txt", "r") as f:
    fr = f.read().strip().split(',')
_class = int(fr[1])
_speed_up = float(fr[2])
...
with open(base+"/Data/3/Wait/"+'Waiting.csv', 'r') as f_object:
    while(True):
...
#to be updated later ----------------------------
if(gpu_run!=0):
    with open(base+"/Data/3/Wait/"+str(gpu_run["Id"])+'.txt', 'w') as f_object:
        f_object.write('1')
        f_object.close()
if(cpu_run!=0):
    with open(base+"/Data/3/Wait/"+str(cpu_run["Id"])+'.txt', 'w') as f_object:
        f_object.write('0')
        f_object.close()
...
with open(base+"/Data/3/Wait/"+'Waiting.csv', 'w') as f_object:
